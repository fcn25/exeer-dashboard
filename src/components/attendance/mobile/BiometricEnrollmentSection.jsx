...
export default function BiometricEnrollmentSection({ employeeId, onEnrollmentChange }) {
  ...
  const handleConfirmEnrollment = async (previewDataUrl) => {
    setIsSaving(true);
    setError("");
  ...
  return (
    ...
    >
      ...
      {enrolled && !isCapturing ? (
          ...
          <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-3 dark:border-emerald-900 dark:bg-emerald-950/30">
            {enrollment?.previewDataUrl ? (
              <img
                ...
                alt=""
                className="h-14 w-14 shrink-0 rounded-full object-cover ring-2 ring-emerald-300 dark:ring-emerald-800"
              />
            ) : (
        ...
        <FaceSelfieCapture
          title={t("pages.mobile.attendance.captureTitle")}
          subtitle={t("pages.mobile.attendance.captureSubtitle")}
          confirmLabel={t("pages.mobile.attendance.saveEnrollment")}
          onConfirm={handleConfirmEnrollment}
          onCancel={enrolled ? () => setIsCapturing(false) : undefined}
          isSaving={isSaving}
        />
      )}
